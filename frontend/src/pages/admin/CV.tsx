...
import { Download, FileText, Info, Loader2, Trash2, Upload } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { Button } from "../../components/ui/Button";
import { Card } from "../../components/ui/Card";
import { useNotification } from "../../context/NotificationContext";
import { queryKeys } from "../../lib/queryKeys";
import { deleteCv, getCV, uploadCv } from "../../services/admin";
...
export function CV() {
    ...
    const uploadMutation = useMutation({
        ...
        onSuccess: res => {
            ...
            const cacheBuster = `?t=${new Date().getTime()}`;
            setPreviewUrl(`${res.data.cvUrl}${cacheBuster}`);
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
                <Card>
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h3 className="font-heading font-semibold text-base text-zinc-100">Vista previa del CV</h3>
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-zinc-400 mt-1">
                                        {cv
                                            ? `${(cv.fileSize / 1024).toFixed(0)} KB · PDF`
            ...
            <div className="space-y-6">
                <Card>
                    <h3 className="font-heading font-semibold text-base mb-5 text-zinc-100">Información del archivo</h3>
                    {cv ? (
                        <div className="space-y-4">
                            {[
                                ...
                                { label: "Páginas", value: String(cv.pages) },
                                {
                                    ...
                                    value: `Hace ${Math.floor((Date.now() - new Date(cv.lastUpdated).getTime()) / 86400000)} días`,
                                },
                            ].map(item => (
                ...
                <Card className="border-cyan-500/30 bg-cyan-500/10 [&>div]:bg-transparent [&>div]:border-0">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-zinc-100">Formato recomendado</p>
                            <p className="text-xs mt-1 leading-relaxed text-zinc-400">
                                Sube tu CV en formato PDF, máximo 10MB. Se mostrará como enlace de descarga en tu
                                portafolio.
                            </p>
                        </div>
                    </div>
